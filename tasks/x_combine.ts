...
module validate {
    ...
    /* Check a file exists */
    export function exists(grunt:any, target:any):boolean {
    ...
    export function is_dir(grunt, target:any):boolean {
...
function task(grunt) {
    ...
    grunt.registerMultiTask('x_combine', 'A template based grunt file combiner.', function () {
        ...
        this.files.forEach(function (f) {
            ...
            // Validate config
            if (!validate.exists(grunt, f.dest)) { return false; }
            if (!validate.exists(grunt, f.template)) { return false; }
            ...
            f.src.forEach(function (target) {
                ...
                if (!validate.exists(grunt, target)) { return false; }
                if (!validate.is_dir(grunt, target)) { return false; }

                // Find all subtarget in the directory
                var matches = glob.sync(target+"/*", {sync: true});
                var keys = matches.map((x) => { return path.basename(x).replace(/\./g, '_'); });
